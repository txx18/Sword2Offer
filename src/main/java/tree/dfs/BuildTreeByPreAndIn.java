...
public class BuildTreeByPreAndIn {
    ...
    public TreeNode reConstructBinaryTree(int[] preorder, int[] inorder) {
        ...
        return preorder(preorder, 0, preorder.length - 1, 0);
    }

    private TreeNode preorder(int[] pre, int preL, int preR, int inL) {
        if (preL > preR) {
            return null;
        }
        ...
        root.left = preorder(pre, preL + 1, preL + leftSize, inL);
        root.right = preorder(pre, preL + leftSize + 1, preR, rootIndexOfIn + 1);
        return root;
    }
